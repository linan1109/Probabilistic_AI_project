'''
Author: Li Nan
Date: 2022-09-28 19:48:36
LastEditors: Li Nan
LastEditTime: 2022-09-28 23:53:11
FilePath: \task0_handout\solution.py
'''
import numpy
...
def log_posterior_probs(x):
    ...
    for dist in HYPOTHESIS_SPACE:
        log_p_x_H = np.append(log_p_x_H, np.sum(dist.logpdf(x)))

    # log(P(H))
    log_p_H = np.log(PRIOR_PROBS)

    # log(P(x))
    log_p_x = logsumexp(log_p_x_H + log_p_H)

    # log(P(Hi|x)) = log(P(x|Hi)) + log(P(Hi)) - log(P(x))
    log_p = log_p_H + log_p_x_H - log_p_x
